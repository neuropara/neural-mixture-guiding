...
template <typename T>
class GuidingNetwork : public Network<float, T> {
public:
	GuidingNetwork(std::shared_ptr<Encoding<T>> encoding, uint32_t n_output_dims, const json& network) : m_encoding{ encoding } {
	...
	GuidingNetwork(uint32_t n_dims_to_encode, uint32_t n_output_dims, const json& encoding, const json& network)
		: GuidingNetwork{ std::shared_ptr<Encoding<T>>{create_encoding<T>(n_dims_to_encode, encoding)}, n_output_dims, network } { }

	virtual ~GuidingNetwork() { }
